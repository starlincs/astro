...
`astro ingest` prints logs to the console and writes them to `.working/{run_id}/astro.log`. `astro run` executes registered pipeline steps; it uses a Rich dashboard by default (`--mode cli` for plain log output). Steps use `AstroFileSpec` / `AstroFile` containers with explicit output paths. Re-run `astro run` against a `quarantined` run to merge quarantined rows back and retry only the affected steps. Record custom metrics from steps via `ctx.stats`.
